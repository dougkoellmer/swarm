...
import swarm.shared.json.bhMutableJsonQuery;
...
public class bhCellCodeManager implements bhI_TransactionResponseHandler
{
				...
				if( !localHtmlSource.tryPopulatingCell(absCoord, eType, cell) )
				{
					if( communicateWithServer )
					{
						...
						if( !txnMngr.containsDispatchedRequest(bhE_RequestPath.getCode, m_getQuery) )
						{
							...
							sm.jsonFactory.getHelper().putInt(request.getJson(), bhE_JsonKey.codeType, eType.ordinal());
	...
	private void onGetCellDataSuccess(bhTransactionRequest request, bhTransactionResponse response)
	{
		...
		int typeOrdinal = sm.jsonFactory.getHelper().getInt(request.getJson(), bhE_JsonKey.codeType);
		...
		else if( request.getPath() == bhE_RequestPath.syncCode )
		{
			...
			if( result.getStatus() == bhE_CompilationStatus.NO_ERROR )
			{
				...
				else
				{
					...
					sm_c.codeCache.cacheCode(m_utilCoord, sourceCode, bhE_CodeType.SOURCE);
					sm_c.codeCache.cacheCode(m_utilCoord, splashScreenCode, bhE_CodeType.SPLASH);
					sm_c.codeCache.cacheCode(m_utilCoord, compiledCode, bhE_CodeType.COMPILED);
				}
