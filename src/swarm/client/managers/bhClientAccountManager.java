...
import swarm.client.managers.bhCellCodeManager.I_SyncOrPreviewDelegate;
...
public class bhClientAccountManager implements bhI_TransactionResponseHandler
{
	...
	public void init(bhI_Callback callback)
	{
		...
		sm_c.txnMngr.performAction(bhE_TransactionAction.QUEUE_REQUEST, bhE_RequestPath.getAccountInfo);
		sm_c.txnMngr.performAction(bhE_TransactionAction.QUEUE_REQUEST_AND_FLUSH, bhE_RequestPath.getPasswordChangeToken);
	}
	...
	public void setNewDesiredPassword(bhSignInCredentials creds)
	{
		...
		sm_c.txnMngr.makeRequest(bhE_RequestPath.setNewDesiredPassword, creds);
	}
	...
	public void signIn(bhSignInCredentials credentials, bhE_TransactionAction action)
	{
		...
		if( m_passwordChangeToken != null )
		{
			sm.jsonFactory.getHelper().putString(request.getJson(), bhE_JsonKey.passwordChangeToken, m_passwordChangeToken);
		}
		
		sm_c.txnMngr.performAction(bhE_TransactionAction.QUEUE_REQUEST, request);
		
		action = action == bhE_TransactionAction.MAKE_REQUEST ? bhE_TransactionAction.QUEUE_REQUEST_AND_FLUSH : action;
		sm_c.txnMngr.performAction(action, bhE_RequestPath.getAccountInfo);
	}
	...
	public bhE_ResponseSuccessControl onResponseSuccess(bhTransactionRequest request, bhTransactionResponse response)
	{
		...
		else if( request.getPath() == bhE_RequestPath.getPasswordChangeToken )
		{
			...
			m_passwordChangeToken = sm.jsonFactory.getHelper().getString(response.getJson(), bhE_JsonKey.passwordChangeToken);
