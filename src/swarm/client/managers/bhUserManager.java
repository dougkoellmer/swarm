...
public class bhUserManager implements bhI_TransactionResponseHandler, bhClientAccountManager.I_Delegate, bhI_ResponseBatchListener
{
	...
	{
		sm_c.txnMngr.performAction(action, bhE_RequestPath.getStartingPosition);
	}
	
	public void populateUser(bhE_TransactionAction action)
	{
		sm_c.txnMngr.performAction(action, bhE_RequestPath.getUserData);
	}
	...
	private void onGetUserDataSuccess(bhTransactionResponse response)
	{
		...
		Boolean createdUser = sm.jsonFactory.getHelper().getBoolean(response.getJson(), bhE_JsonKey.createdUser);
		...
		while( cellIterator.hasNext() )
		{
			...
			if( createdUser )
			{
				...
				bhCellCodeManager codeManager = sm_c.codeMngr;
	...
	private void clearUser()
	{
		...
		if( user.isPopulated() )
		{
			sm_c.txnMngr.cancelRequestsByPath(bhE_RequestPath.getUserData);
