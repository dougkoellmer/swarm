...
{	
	...
	protected void startUpCoreUI()
	{
		...
		VisualCellFocuser focuser = new VisualCellFocuser(m_viewContext.stateContext, m_viewContext.appContext);
	...
	{
		if ( event.getType() == E_StateEventType.DID_ENTER )
		{
		...
		else if( event.getType() == E_StateEventType.DID_UPDATE )
		{
			if( event.getState() instanceof StateMachine_Base )
			{
