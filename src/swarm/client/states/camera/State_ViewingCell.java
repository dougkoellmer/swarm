...
public class State_ViewingCell extends A_State implements I_StateEventListener, I_State_SnappingOrViewing
{
	...
	void refreshCell()
	{
		...
		{
			codeManager.populateCell(cell, localCodeRepo, 1, true, /*populateFromLocal=*/true, E_CodeType.SOURCE);
		...
		codeManager.populateCell(cell, localCodeRepo, 1, true, /*populateFromLocal=*/true, E_CodeType.SPLASH);
		codeManager.populateCell(cell, localCodeRepo, 1, true, /*populateFromLocal=*/true, E_CodeType.COMPILED);
	...
	private void requestSourceHtmlForTargetCell()
	{
		...
		if( !m_hasRequestedSourceCode )
		{
			if( getContext().isForegrounded(StateMachine_EditingCode.class) )
			{
				...
				codeMngr.populateCell(m_cell, localHtmlSource, 1, true, /*populateFromLocal=*/true, E_CodeType.SOURCE);
