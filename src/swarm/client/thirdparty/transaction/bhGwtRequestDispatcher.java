...
import swarm.client.transaction.bhI_AsyncRequestDispatcher;
import swarm.client.transaction.bhI_ResponseCallbacks;
import swarm.client.transaction.bhTransactionRequestBatch;
...
public class bhGwtRequestDispatcher implements bhI_AsyncRequestDispatcher, RequestCallback
{
	...
	public void onResponseReceived(Request nativeRequest, Response nativeResponse)
	{
		...
		bhI_JsonObject responseJson = sm.jsonFactory.createJsonObject(nativeResponse.getText());
		...
		else
		{
			...
			bhI_JsonArray responseList = sm.jsonFactory.getHelper().getJsonArray(responseJson, bhE_JsonKey.responseList);
