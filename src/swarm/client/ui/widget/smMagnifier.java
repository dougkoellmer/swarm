...
public class smMagnifier extends FlowPanel implements smI_StateEventListener
{
	...
	public smMagnifier(double tickCount, double fadeInTime_seconds)
	{
		...
		sm_c.clickMngr.addClickHandler(m_zoomIn, new smI_ClickHandler()
		{
			public void onClick()
			{
		...
		sm_c.clickMngr.addClickHandler(m_zoomOut, new smI_ClickHandler()
		{
			public void onClick()
			{
	...
	private void onDraggerMouseUp(HumanInputEvent event, double relativeY)
	{
		if( m_isMouseDown )
		{
			...
			{
				sm_c.toolTipMngr.onMouseUp((MouseUpEvent)event);
			}
