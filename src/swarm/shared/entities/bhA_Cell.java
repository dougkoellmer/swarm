...
public abstract class bhA_Cell extends bhA_JsonEncodable
{
	...
	public void writeJson(bhI_JsonObject json)
	{
		...
		for( int i = 0; i < bhE_CodeType.values().length; i++ )
		{
			if( m_code[i] != null )
			{
				sm.jsonFactory.getHelper().putJsonObject(json, bhE_CodeType.values()[i].getJsonKey(), m_code[i].writeJson());
			}
		}
	}
	...
	public void readJson(bhI_JsonObject json)
	{
		...
		for( int i = 0; i < bhE_CodeType.values().length; i++ )
		{
			...
			bhI_JsonObject jsonForCode = sm.jsonFactory.getHelper().getJsonObject(json, key);
