...
public class bhBitArray extends bhA_JsonEncodable
{
	...
	public void writeJson(bhI_JsonObject json)
	{
		if( m_blocks != null )
		{
			bhI_JsonArray blocksAsJson = sm.jsonFactory.createJsonArray();
			for( int i = 0; i < m_blocks.length; i++ )
			{
				blocksAsJson.addInt(m_blocks[i]);
			}
			
			sm.jsonFactory.getHelper().putJsonArray(json, bhE_JsonKey.bitArray, blocksAsJson);
			sm.jsonFactory.getHelper().putInt(json, bhE_JsonKey.bitArrayLength, m_bitCount);
		}		
	}
	...
	{
		Integer bitCount = sm.jsonFactory.getHelper().getInt(json, bhE_JsonKey.bitArrayLength);
		
		if( bitCount == null )  return;
		
		this.init(bitCount);
		
		bhI_JsonArray blocksAsJson = sm.jsonFactory.getHelper().getJsonArray(json, bhE_JsonKey.bitArray);
